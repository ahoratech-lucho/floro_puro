export default function DownloadSection() {
  return (
    <section className="py-20 px-6" id="descargar">
      <div className="max-w-2xl mx-auto text-center">
        <h2 className="text-3xl font-bold text-white mb-4">
          Descarga el juego
        </h2>
        <p className="text-gray-400 mb-8">
          Disponible para Android. Sin internet después de la primera carga.
        </p>

        <a
          href="/downloads/radar-del-floro.apk"
          className="inline-flex items-center gap-3 bg-red-600 hover:bg-red-700 text-white font-bold py-4 px-10 rounded-2xl text-lg transition-all transform hover:scale-105 shadow-lg shadow-red-600/30"
        >
          <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.523 2.235a.5.5 0 0 0-.866-.015l-1.69 2.86a8.5 8.5 0 0 0-5.934 0L7.343 2.22a.5.5 0 0 0-.866.015.5.5 0 0 0 .046.488l1.618 2.74A8.5 8.5 0 0 0 3.5 12.5h17a8.5 8.5 0 0 0-4.641-7.037l1.618-2.74a.5.5 0 0 0 .046-.488zM8.5 9.5a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm7 0a1 1 0 1 1 0 2 1 1 0 0 1 0-2zM3.5 13.5v6A2.5 2.5 0 0 0 6 22h12a2.5 2.5 0 0 0 2.5-2.5v-6h-17z"/>
          </svg>
          Descargar APK para Android
        </a>

        <div className="mt-8 grid grid-cols-3 gap-4 max-w-sm mx-auto">
          <InfoPill label="~15 MB" sub="Tamaño" />
          <InfoPill label="Android 5+" sub="Compatible" />
          <InfoPill label="Gratis" sub="Precio" />
        </div>

        <div className="mt-8 p-4 bg-gray-900/50 rounded-xl border border-gray-800">
          <p className="text-gray-500 text-xs">
            <strong className="text-gray-400">Nota:</strong> Al instalar, Android
            pedirá permiso para instalar apps de fuentes desconocidas. Esto es
            normal para APKs que no vienen de Google Play.
          </p>
        </div>
      </div>
    </section>
  );
}

function InfoPill({ label, sub }: { label: string; sub: string }) {
  return (
    <div className="text-center">
      <div className="text-white font-bold text-sm">{label}</div>
      <div className="text-gray-600 text-xs">{sub}</div>
    </div>
  );
}
